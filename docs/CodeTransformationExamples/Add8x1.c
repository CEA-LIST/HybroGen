...
code * genAdd(insn * code, tarch a, tvariant v) // C compilette prototype
{
  if ((a == RISCV) && (v == RV32F))
...
{ // Compilette rewriting Add8x16.hl 
  ...
  gen_add_RISCV_RV32I_XfR8(sValue Dest, sValue S1, sValue S2)
    { // Only add 2 vectors of 16 elements of 8 bits floats
      ...
      if (Dest.Arith == "f" && Dest.Vlen == 1 && Dest.Wlen == 8 && ValOrReg == REG)
       {
  ...
  // Compilette generator
	code *  add (flt 8 1 , flt 8 1 b)
	{
      gen_add_RISCV_RV32I_RV32F(RISCV_out0, RISCV_inf0, RISCV_in1);
      gen_return();
    }
...
int main(int argv, char * argv)
{
  ...
  code = gen_add(ptr, RISCV, RV32F); // Generate instructions

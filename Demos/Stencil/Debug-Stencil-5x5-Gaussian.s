...
(gdb)
0x760:  sub w4,  w2, #0x2     
0x764:  mv  NJM2, w4                  						; w11/njm2 <- column - 2
0x768:  sub w4,  w2, #0x1
0x76c:  mv  NJM1, w4                  						; w12/njm1 <- column - 1
0x770:  add w4,  w2, #0x1
0x774:  mv  NJP1, w4                  						; w13/njp1 <- column + 1
0x778:  add w4,  w2, #0x2
0x77c:  mv  w14, w4                  						; w14/njp2 <- column + 2
0x780:  sub w4,  w1, #0x2    
0x784:  mv  w10, w4                  						; w10/ni = line - 2
0x788:  mul w4,  w10, w3
0x78c:  mv  w15, w4                  						; w15/stride = ni * imgWidth
0x790:  add w4,  w15, NJM2/lsl x5,  x4, #2/add x4, x0, x5/ldr w5, [x4]          w5 = ld In[stride+njm2] 
0x7a0:  mv  w9,  w5                   						; w9 = In[strinde+njm2] * 1

0x7a4:  add w4, w15,  NJM1/lsl x5, x4, #2/add x4, x0, x5/ ldr w5, [x4]
0x7b4:  lsl w6, w5, #2
...
0x7e0:  add  w4, w15, NJP1/lsl  x5, x4, #2/add  x4, x0, x5/ldr  w5, [x4]
0x7f0:  lsl  w6, w5, #2              						; w6 = In[strinde+njm2] * 4
0x7f4:  add  w5, w9, w6
0x7f8:  mv   w9, w5
	
0x7fc:  add  w4, w15, NJP1/lsl  x5, x4, #2/add  x4, x0, x5/ldr  w5, [x4] ; <-- should be NJP2 !
0x80c:  add  w6, w9, w5
0x810:  mv   w9, w6
...
0x824:  add  w4, w15, NJM2/lsl  x5, x4, #2/ add  x4, x0, x5 / ldr  w5, [x4]
0x834:  lsl  w6, w5, #2        								; * 16
0x854:  add  w5, w9, w6
0x858:  mv   w9, w5
...
0x87c:  add  w4, w15, NJP1/lsl  x5, x4, #2/add  x4, x0, x5/ldr  w5, [x4]
